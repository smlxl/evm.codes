importScripts('https://solc-bin.ethereum.org/bin/soljson-v0.8.9+commit.e5eed63a.js')
...
const solidityCompile = cwrap('solidity_compile', 'string', ['string', 'number', 'number'])
...
onmessage = function (e) {
  ...
  const output = JSON.parse(compile(JSON.stringify(input), { import: findImports }))
  let warningMessage = ''
  ...
  for (const contractName in output.contracts.web) {
    if (!assembly) {
      assembly = output.contracts.web[contractName].evm.bytecode.object
    }
    else {
      postMessage({ error: 'The source should contain only one contract' })
      return
    }
  }
  ...
  if (!assembly) {
    ...
    postMessage({ error: null, warning: warningMessage, code: assembly })
  }
}
