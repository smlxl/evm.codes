...
import { IPrecompiled } from 'types'

import { EthereumContext } from 'context/ethereumContext'

import { calculatePrecompiledDynamicFee } from 'util/gas'
...
const DynamicFee = ({ precompiled, fork }: Props) => {
  const { dynamicFee } = precompiled
  ...
  const handleCompute = debounce((inputs) => {
    if (common) {
      try {
        setResult(calculatePrecompiledDynamicFee(precompiled, common, inputs))
      } catch (error) {
        console.error(error)
      }
    }
