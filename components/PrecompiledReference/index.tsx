...
import { IPrecompiled, IPrecompiledDocs, IPrecompiledGasDocs } from 'types'
...
type CustomHeaderGroup = {
  className?: string
} & HeaderGroup<IPrecompiled>

const PrecompiledReferenceTable = ({
  precompiledDocs,
  gasDocs,
}: {
  ...
  const { precompiled, forks, selectedFork } = useContext(EthereumContext)
  const data = useMemo(() => precompiled, [precompiled])
  ...
  useEffect(() => {
    if (precompiled && rowRefs?.current) {
      const idx = precompiled.findIndex((precompiled) => {
        const re = new RegExp(`/#${precompiled.address}`, 'gi')
              ...
              <Fragment key={row.getRowProps().key}>
                ...
                {isExpanded ? (
                  <tr className="bg-indigo-50 dark:bg-black-600">
                    <td colSpan={colSpan}>
                      <DocRow
                        precompiledDoc={precompiledDocs[address]}
                        precompiled={precompiled[rowId]}
                        gasDocs={gasDocs[address]}
...
export default PrecompiledReferenceTable
