...
Pop from the stack a value as `offset` to locate and read a `uint256` word from the memory at `offset` position and push into top of stack. Expand memory with uint256(0) to the `offset` position if necessary.
