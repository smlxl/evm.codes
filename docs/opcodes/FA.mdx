...
2. `Integer`: offset in the [memory](/about) in bytes, the [calldata](/about) of the sub [context](/about).
3. `Integer`: how many bytes to copy (size of the [calldata](/about)).
4. `Integer`: offset in the [memory](/about) in bytes, where to store the [return data](/about) of the sub [context](/about).
5. `Integer`: how many bytes to copy (size of the [return data](/about)).
